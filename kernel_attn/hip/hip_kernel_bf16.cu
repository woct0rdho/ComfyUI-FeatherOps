...
extern "C" bool launch_attn_bf16(
    ...
    const int64_t N_WAVES)
{
    ...
    const bool launched = launch_attn_bf16(
        q_ptr,
        k_ptr,
        v_ptr,
        o_ptr,
    ...
    STD_TORCH_CHECK(launch_err == hipSuccess, "attn_bf16 kernel launch failed: ", hipGetErrorString(launch_err));
}
...
{
    m.impl("attn_bf16", TORCH_BOX(&attn_bf16));
}
#endif
