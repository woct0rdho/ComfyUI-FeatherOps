...
- Graph path at `N=8192` is 3 kernels, not 1 fused kernel:
  - Triton fp8->fp16 cast (`triton_poi_fused__to_copy_0`)
  - GEMM (`extern_kernels.mm` -> ATen -> rocBLAS/Tensile asm kernel)
  - Triton fused epilogue (`mul(scale)+add(bias)`)
...
- `triton_poi_fused__to_copy_0`: converts `b` from fp8 (`f8e4m3fn`) to fp16 into an intermediate `buf0`.
- `extern_kernels.mm(arg0_1, buf0, out=buf1)`: GEMM.
- `triton_poi_fused__to_copy_add_mul_1`: fused `mul(scale)` + `add(bias)` epilogue on `buf1`.
...
- Therefore occupancy is VGPR-limited at `min(6, 7, 16) / 16 = 37.5%`, i.e. 6 resident waves per SIMD.
...
- torch.compile currently does a full-tensor fp8->fp16 conversion before GEMM.
- That conversion kernel moves `201 MB` per iteration (`N=8192`) and runs in `0.95 ms` (`212 GB/s`), i.e. memory-bandwidth heavy.
- There is a separate post-GEMM fused epilogue kernel (`1.04 ms`).
- GEMM itself is a heavily tuned wave32 Tensile kernel with large tile (`128x128x32`), high vectorization, and moderate occupancy (`37.5%`).
...
- fp8 load/convert scheduling inside the GEMM mainloop.
- local read vectorization behavior (torch symbol indicates `LRVW16`).
- wave-separated/staggered global read behavior (`WSGRA1`, `WSGRB1`, `SU32`, `SUS256`, `PGR1`).
...
Through thread tracing of the Tensile FP16 kernel generated by `torch.compile`, we identified exactly why it hits 32 TFLOPS (53% theoretical peak), while our custom FP8 mixed precision kernel hits 44 TFLOPS (75% theoretical peak).
...
- LDS Overhead: Extracting cycle timings from the hardware trace shows that the Tensile kernel spends 35% of its active compute time just reading from LDS (`ds_read` operations) and calculating pointer arithmetic (`v_add`).
- WMMA Starvation: Because the dispatch pipe is clogged with thousands of narrow, unoptimized LDS reads and pointer instructions, the matrix cores (`WMMA`) are only active for 65% of the math loop.
  - Maximum possible TFLOPS due to LDS overhead: `59.4 TFLOPS * 65% = 38.6 TFLOPS`.
...
- That limits each SIMD to `floor(1536 / 256) = 6` resident waves.
- With a 4-wave workgroup, this is still only 37.5% occupancy, but it is not a collapse to a single resident workgroup with no other waves available.

Because occupancy is materially below our HIP kernel's 8 waves/SIMD path, Tensile has less latency-hiding headroom when its math loop stalls, even though multiple resident waves still remain available.
