...
from kernel_attn.hip.hip_kernel_bf16 import _CONFIGS, attn_hip_bf16_configured
...
def test_config(cfg, B, H, N, N_KV, D, device):
    ...
    out_hip = attn_hip_bf16_configured(q, k, v, *cfg)

    out_hip = out_hip.float()
    out_ref = out_ref.float()
    diff = out_hip - out_ref
    l2_rel_err = diff.norm() / out_ref.norm().clamp_min(1e-6)
...
def main():
    ...
    print(f"Testing attention HIP bf16 kernel ({len(_CONFIGS)} configs across {len(test_sizes)} matrix sizes)\n")
