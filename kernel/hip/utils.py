...
def _canonicalize_scale_bias(a: torch.Tensor, n: int, scale: Optional[torch.Tensor], bias: Optional[torch.Tensor], out_dtype: torch.dtype) -> tuple[torch.Tensor, torch.Tensor]:
    scale_t = torch.empty(0, device=a.device, dtype=out_dtype) if scale is None else scale.to(out_dtype)
    bias_t = torch.empty(0, device=a.device, dtype=out_dtype) if bias is None else bias.to(out_dtype)
    return scale_t, bias_t


def _canonicalize_bias(a: torch.Tensor, n: int, bias: Optional[torch.Tensor], out_dtype: torch.dtype) -> torch.Tensor:
    return torch.empty(0, device=a.device, dtype=out_dtype) if bias is None else bias.to(out_dtype)
