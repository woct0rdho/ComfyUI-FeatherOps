    ...
    const int has_bias)
{
    ...
    union SharedStorage {
        struct {
            half a[kShASize];
            half b[kUnrollK][kWmmaK][kBlockN];
        } ab;
    ...
    const auto load_b_lds = [&](const int stage, const int64_t kk) -> void {
        // Load B with vec16 fp8->fp16 conversion, store to KxN layout
        #pragma unroll
        for (int v = 0; v < kBVecsPerThread; ++v) {
            ...
            uint4* const dst_ptr = reinterpret_cast<uint4*>(&sh.ab.b[stage][row][col]);
    ...
    const auto wmma_compute_stage = [&](const int stage) -> void {
        ...
        for (int rn = 0; rn < kRepeatN; ++rn) {
            ...
            for (int k = 0; k < kWmmaK; ++k) {
                all_reg_b[rn][k] = static_cast<_Float16>(sh.ab.b[stage][k][n_col]);
            }
        }
    ...
    const int64_t repeat_n)
{
    ...
    TORCH_CHECK(K == b.size(0), "a and b shapes are incompatible");
