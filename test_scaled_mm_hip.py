...
def run_case(M, N, K, device, out_dtype, with_scale, with_bias, kernel_fn, kernel_name):
    ...
    out_hip = kernel_fn(a, b, scale, bias, out_dtype)
    ...
    print(
        f"[{kernel_name}] M={M} N={N} K={K} out={out_dtype} scale={with_scale} bias={with_bias} "
...
def main():
    ...
    # Test K0MK1 kernel
    print("=== Testing K0MK1 kernel ===")
    run_case(64, 64, 64, device, out_dtype, with_scale=True, with_bias=True, kernel_fn=scaled_mm_hip, kernel_name="K0MK1")
    run_case(128, 96, 160, device, out_dtype, with_scale=True, with_bias=False, kernel_fn=scaled_mm_hip, kernel_name="K0MK1")
    run_case(192, 128, 80, device, out_dtype, with_scale=False, with_bias=True, kernel_fn=scaled_mm_hip, kernel_name="K0MK1")
