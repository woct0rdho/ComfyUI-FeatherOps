...
However, this is helpful only when the tensors are small. When the tensors go larger, the computation time goes by O(N^3), while the communication time only goes by O(N^2). It cannot achieve speedup on Z-Image and larger models, which are compute-bound rather than communicate-bound.
